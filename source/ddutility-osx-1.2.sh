#!/bin/bash
#
# dd Utility version 1.2 - Mac OS X 
#
...
function getdevdisk () {
  ...
  while [ $countdev -eq 0 ] ; do
    osascript -e 'display notification "Insert memory card" with title "'"$apptitle"'" subtitle "No Volumes Detected"'
    # Ask for redetect
  ...
  devdisk=$( echo $devselect | grep 'Disk' | awk -F 'Disk\ ' '{print "/dev/disk"$2}' )
...
disknum=$( diskutil list | grep "$devdisk" | awk -F 'disk' '{print $2}' | cut -d 's' -f1 )
...
echo "PROGRESS:100"
response=$(osascript -e 'tell app "System Events" to display dialog "Please confirm settings and click Start\n\nSource: \n'"$source"' \n\nDestination: \n'"$dest"' \n\n'"$warning"'\n\nNOTE: All Data on the Destination will be deleted and overwritten" buttons {"Cancel", "Start"} default button 2 with title "'"$apptitle"' - '"$action"'" with icon POSIX file "'"$iconfile"'" ')
...
if [ "$compression" == "Yes" ] ; then
  ...
  if [ "$action" == "Backup" ] ; then
    dd if="$inputfile" bs=1m | zip > "$outputfile" &
  fi
...
osascript -e 'display notification "You can remove you memory card now" with title "'"$apptitle"'" subtitle "'"$drivesizehuman"' Volume '"$action"' Complete"'

response=$(osascript -e 'tell app "System Events" to display dialog "'"$drivesizehuman"' Volume '"$action"' Complete" buttons {"Done"} default button 1 with title "'"$apptitle"' '"$version"'" with icon POSIX file "'"$iconfile"'" ')

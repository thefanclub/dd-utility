#!/bin/sh
#
# dd Utility version 1.8 - Mac OS X 
#
...
function getdevdisk () {
  ...
  for (( c=1; c<=$countdev; c++ ))
    do
      ...
      devejectable=$( diskutil info "$devitem" | grep "Ejectable" | cut -d ":" -f2 | xargs)
      ...
      if [ "$devejectable" != "No" ] && [ "$devremoveable" != "No" ] && [ "$devprotocol" != "Disk Image" ] ; then

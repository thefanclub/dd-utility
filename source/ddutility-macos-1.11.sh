...
function progressmonitor () {
  ...
  while [ $processactive ] ; do
    ...
    # Process sometimes nevers tops
    if [ $percentage -gt 101 ] ; then
      break
    fi
  done
}
...
function getdevdisk () {
  ...
  for (( c=1; c<=$countdev; c++ ))
    do
      ...
      # Check for MacOS 10.11
      if [ !$drivesizehuman ] ; then
        drivesizehuman=$( diskutil info "$devitem" | grep "Disk\ Size" | awk '{print $3" "$4}' )
      fi
      devtype=$( diskutil info "$devitem" | grep "Device\ \/\ Media" | cut -d ":" -f2 | xargs)
      #devremoveable=$( diskutil info "$devitem" | grep "Removable\ Media" | cut -d ":" -f2 | xargs)
      ...
      if [ "$devprotocol" == "USB" ] ; then
        # Create List of "item","item","item" for select dialog
        # Add , if not first item
        if [ "$devitems" != "" ] ; then
          devitems=$devitems","
        fi
        # Add item
        printf -v devtype %-40.40s "$devtype" # Magic pad 40 spaces code to align text
  ...
  if [ "$devitems" ] ; then
    ...
    devdisk=$( echo $devselect | grep 'Disk' | rev | cut -d' ' -f1 | xargs | awk '{print "/dev/disk"$1}' | xargs )
...
if [ "$action" == "Restore" ] ; then
  ...
  if [ ! $imagepath ] ; then
    ...
    imagepath=`/usr/bin/osascript << EOT
      tell application "Finder"
        activate
        set imagefilepath to choose file of type {"img", "iso", "gz", "zip", "xz"} default location (path to downloads folder) with prompt "Select image file to restore to memory card or disk. Supported file formats : IMG, ISO, ZIP, GZ, XZ"
      end tell 
...
# Check for MacOS 10.11
if [ ! "$drivesizehuman" ] ; then
  drivesize=$( diskutil info $devdisk | grep "Disk\ Size" | cut -d "(" -f2 | awk {'print $1'})
  drivesizehuman=$( diskutil info $devdisk | grep "Disk\ Size" | awk {'print $3" "$4'} )
fi
...
if [ "$action" == "Restore" ] ; then
  ...
  case "$extension" in
    ...
    zip)
       ...
       ;;
     gz)
       # Changed print $1 to $2 (seems wrong on 10.10.12 compressed size seeems wrong)
       totalbytes=$( gzip -l "$inputfile" | tail -1 | awk '{print $1}')
       ;;
     xz)
...
if [ "$action" == "Restore" ] ; then
  ...
  if [ "$extension" == "img" ] ; then
    dd if="$inputfile" of="$outputfile" bs=16m &
  fi
  # GZ files
  if [ "$extension" == "gz" ] ; then
    gzip -dc "$inputfile" | dd of="$outputfile" bs=4m &
  fi
  # ZIP files
  if [ "$extension" == "zip" ] ; then
    unzip -p "$inputfile" | dd of="$outputfile" bs=4m &    
  fi
  # XZ files - OSX 10.10 only I think 
  if [ "$extension" == "xz" ] ; then
    gunzip -dc "$inputfile" | dd of="$outputfile" bs=4m &
  fi
